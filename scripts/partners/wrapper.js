...
(function ($) {
	...
	var bt_bb_masonry_image_grid_fix_resize = function () {
		...
		$('.bt_bb_masonry_image_grid').each(function () {
			...
			var child_margin =
				parseInt(
					$(this).find('.bt_bb_masonry_post_image_content').css('margin-right')
				) +
				parseInt(
					$(this).find('.bt_bb_masonry_post_image_content').css('margin-left')
				);
			var base_item_width =
				($(this).width() - child_margin) / $(this).data('columns');
			if (Math.ceil(base_item_width) != base_item_width) {
				$(this).width(
					$(this).data('columns') * Math.ceil(base_item_width) + child_margin
				);
			}
		});
	};
				...
				.each(function () {
					...
					if (this_top < page_bottom + $(window).height()) {
						...
						if (
							...
							$(this)
								.find('.bt_bb_grid_item_inner_image')
								.html(
									'<img src="' +
										img_src +
										'" title="' +
