...
(function ($) {
	...
	window.bt_cc_currency_format = function (c, n) {
		...
		Number.prototype.toNumberFormat = function (t, r, e) {
			(r = r || '.'), (e = e || '');
			var o = this.toFixed(t).split('.');
			return (o[0] = o[0].replace(/\B(?=(\d{3})+(?!\d))/g, e)), o.join(r);
		};
			...
			c.find('.wpcf7-submit').on('click', function () {
				...
				if (c.data('currency') != '') {
					if (c.data('currency_after') == 'yes') {
						currency_text =
							c.data('total') + currency_space + c.data('currency');
					} else {
						currency_text =
							c.data('currency') + currency_space + c.data('total');
					}
				} else {
